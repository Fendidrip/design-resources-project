...
// <!-------- filter data based on domain start ------->
  ...
  function filterTable() {
    ...
    let table = document.getElementById("conferenceTable");
...
// <!--- table sorting script start -------------------->
  ...
  rows.sort((a, b) => {
    ...
    // Custom ranking for CORE column
    if (n === 3) {
      const rankOrder = {
        "A*": 1,
        "A": 2,
      ...
      const valA = rankOrder[cellA] || 99;
      const valB = rankOrder[cellB] || 99;
...
// <!--- Expandable Table Script Start -------------------->
  function expandTable() {
    const tableContainer = document.getElementById("academicTablePreview");
    const overlay = document.getElementById("tableOverlay");
    tableContainer.style.maxHeight = "none";
    overlay.style.display = "none";
  }

// <!--- Expandable Table Script End -------------------->
...
function refreshRowNumbers() {
  const tbody = document.getElementById("conferenceTable").getElementsByTagName('tbody')[0];
